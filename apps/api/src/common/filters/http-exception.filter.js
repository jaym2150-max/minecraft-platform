...
function redactHeaders(headers) {
    ...
    for (var _i = 0, _a = Object.entries(headers); _i < _a.length; _i++) {
        ...
        out[name_1] = REDACTED_HEADERS.has(name_1.toLowerCase()) ? '[REDACTED]' : value;
    }
    return out;
}
...
var HttpExceptionFilter = function () {
    ...
    var HttpExceptionFilter = _classThis = /** @class */ (function () {
        ...
        HttpExceptionFilter_1.prototype.catch = function (exception, host) {
            ...
            var safeHeaders = redactHeaders(request.headers);
            this.logger.error("".concat(request.method, " ").concat(request.url, " ").concat(status, " - ").concat(scrubString(exception.message), " ") +
                "headers=".concat(JSON.stringify(safeHeaders)));
