export default function SdkUsagePage() {
  return (
    <article>
      ...
      <pre><code>{`const result = await sdk.listProjects({
  search: 'performance',
  loader: 'FABRIC',
...
for (const project of result.data) {
  console.log(project.title, project.downloads);
      ...
      <pre><code>{`const project = await sdk.createProject({
  ...
  serverSide: false,
      ...
      <pre><code>{`// Step 1: Get a presigned upload URL
...
await sdk.createVersion(projectId, {
  ...
  loaders: ['FABRIC'],
      ...
      <h2>Searching</h2>
      <pre><code>{`const results = await sdk.search('shaders', { page: 1, limit: 10 });
console.log(\`Found \${results.meta.total} results in \${results.meta.processingTimeMs}ms\`);`}</code></pre>
...
function useProjects(filters) {
  return useQuery({
    ...
    queryFn: () => sdk.listProjects(filters).then(r => r.data),
      ...
      <p>
        On the server, you'll want to use a per-user SDK instance with the user's auth token:
      </p>
