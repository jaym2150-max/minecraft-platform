...
# yarn
yarn add @mcp/sdk`}</code></pre>

      <h2>Initialize</h2>
      <pre><code>{`import { McpSDK } from '@mcp/sdk';

const sdk = new McpSDK('https://api.minecraftplatform.com/api/v1');

// Or for local development
const devSdk = new McpSDK('http://localhost:4000/api/v1');`}</code></pre>

      <h2>Authenticate</h2>
      <pre><code>{`const result = await sdk.login('user@example.com', 'password123');
...
sdk.clearAuthToken();`}</code></pre>

      <h2>Error handling</h2>
      <p>
        The SDK throws <code>ApiError</code> on non-2xx responses:
      </p>
      <pre><code>{`import { ApiError } from '@mcp/utils/api-client';
...
try {
  ...
  if (err instanceof ApiError) {
    if (err.statusCode === 401) {
      ...
      <pre><code>{`import { Project, User, ProjectVersion, LoaderType } from '@mcp/types';

const project: Project = await sdk.getProject('sodium').then(r => r.data);`}</code></pre>
    </article>
  );
}
