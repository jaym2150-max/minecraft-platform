...
import {
  Wallet, TrendingUp, Clock, Coins, ArrowUpRight, ExternalLink,
...
export function EarningsPanel() {
  ...
  const connect = useMutation({
    ...
    onSuccess: (data) => {
      if (data?.url) {
        ...
        toast.info('Payouts are not configured on this environment yet — your balance is still tracked.');
      }
      summary.refetch();
    },
  ...
  if (summary.error || !summary.data) {
    return (
      <div className="rounded-xl border border-destructive/20 bg-destructive/5 p-8 text-center text-sm text-destructive flex items-center justify-center gap-2">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { icon: Coins, label: 'Available to withdraw', value: usd(s.availableCents), accent: true },
          ...
          { icon: TrendingUp, label: 'Reward points', value: formatNumber(s.pointsBalance), accent: false },
        ].map((c) => {
          const Icon = c.icon;
          return (
            <div key={c.label} className={`rounded-xl border p-5 ${c.accent ? 'border-primary/30 bg-primary/5' : 'bg-card'}`}>
              <div className="mb-3 inline-flex h-10 w-10 items-center justify-center rounded-lg bg-primary/10">
                <Icon className="h-5 w-5 text-primary" />
              </div>
              <p className="text-xs uppercase tracking-widest text-muted-foreground">{c.label}</p>
              <p className={`mt-1 text-xl font-black tracking-tight ${c.accent ? 'text-primary' : ''}`}>{c.value}</p>
            </div>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-4 rounded-xl border bg-card p-6">
        <div>
          ...
          {s.payoutConnected ? (
            s.payoutsEnabled ? (
              ...
              <p className="mt-1 inline-flex items-center gap-2 text-sm text-amber-600 dark:text-amber-400">
                <AlertTriangle className="h-4 w-4" /> Onboarding incomplete — finish setup to withdraw
              </p>
            )
          ) : (
            <p className="mt-1 text-sm text-muted-foreground">
              Connect a Stripe account to receive payouts. Minimum withdrawal {usd(s.minWithdrawalCents)}.
            </p>
          )}
        </div>
        <div className="flex items-center gap-3">
          {!s.payoutConnected && (
            <Button onClick={() => connect.mutate()} disabled={connect.isPending} className="gap-2">
              {connect.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <ArrowUpRight className="h-4 w-4" />}
              Connect Stripe
            </Button>
          )}
          {s.payoutConnected && !s.payoutsEnabled && (
            <Button variant="outline" onClick={() => connect.mutate()} disabled={connect.isPending} className="gap-2">
              Finish Setup <ExternalLink className="h-3.5 w-3.5" />
            </Button>
          )}
          <Button
            onClick={() => withdraw.mutate()}
            disabled={withdraw.isPending || !s.payoutConnected || s.availableCents < s.minWithdrawalCents}
            title={s.availableCents < s.minWithdrawalCents ? `Minimum ${usd(s.minWithdrawalCents)}` : undefined}
            className="gap-2"
          >
      ...
      <div className="rounded-xl border bg-card p-6">
        ...
        {payouts.isLoading ? (
          ...
          <div className="py-8 text-center">
            <Coins className="mx-auto mb-2 h-8 w-8 text-muted-foreground/40" />
            <p className="text-sm text-muted-foreground">No payouts yet. Withdrawals appear here.</p>
          </div>
        ) : (
