...
const ENDPOINTS = [
  {
    ...
    desc: 'Search & filter projects (type, category, loader, game version, sort)',
  },
  {
    method: 'GET',
    path: '/api/v1/projects/:slug',
    desc: 'Full project detail incl. gallery, license, team',
  },
  {
    method: 'GET',
    path: '/api/v1/projects/:id/versions',
    desc: 'All versions with loaders + game versions',
  },
  {
    method: 'GET',
    path: '/api/v1/versions/:id/download',
    desc: 'Pre-signed download URL for a version file',
  },
  {
    method: 'POST',
    path: '/api/v1/version-files/bulk',
    desc: 'Resolve SHA-256 hashes → versions (launcher sync)',
  },
  {
    method: 'GET',
    path: '/api/v1/version-files/:hash',
    desc: 'Hash lookup (single file compatibility check)',
  },
  {
    method: 'GET',
    path: '/api/v1/projects/:slug/modpack/download',
    desc: '.mrpack archive — importable into Modrinth App / Prism',
  },
      ...
      <main className="flex-1">
        ...
        <section className="bg-muted/40 border-b">
          <div className="container py-12">
            <span className="text-primary inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-black tracking-widest">
            ...
            <p className="text-muted-foreground mt-3 max-w-2xl">
              The same API that powers this site — free for launchers, mod managers, Discord bots
              and analytics tools. JSON over HTTP, cursor pagination, hash-based file resolution.
            </p>
        ...
        <section className="container grid gap-8 py-10 lg:grid-cols-[1fr_1.2fr]">
          <div className="space-y-5">
              ...
              {[
                {
                  ...
                  d: 'Sign up and open your dashboard to generate an API key.',
                },
                {
                  icon: ShieldCheck,
                  t: '2. Scope your key',
                  d: 'Pick read scopes (projects, versions) or write scopes for uploads.',
                },
                {
                  icon: Terminal,
                  t: '3. Call the API',
                  d: 'Pass your key as Authorization: Bearer <key>. 60 req/min free tier.',
                },
              ].map((s) => {
                const Icon = s.icon;
                return (
                  <li key={s.t} className="flex gap-3">
                    <span className="bg-primary/10 text-primary mt-0.5 inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg">
            ...
            <div className="bg-card rounded-xl border p-4">
              <p className="text-muted-foreground mb-2 flex items-center gap-2 text-xs font-black tracking-widest">
                <Zap className="text-primary h-3.5 w-3.5" /> RATE LIMITS
              </p>
              <ul className="text-muted-foreground space-y-1 text-sm">
                <li>• Anonymous: 60 req/min per IP</li>
                <li>• API key: 300 req/min</li>
                <li>
                  • Headers: <code>x-ratelimit-limit / -remaining / -reset</code>
                </li>
              </ul>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="overflow-hidden rounded-xl border bg-zinc-950 text-zinc-100">
              ...
              <pre className="overflow-x-auto p-4 text-xs leading-6">
                <code>{`# Search performance mods for MC 1.21
...
curl -X POST http://localhost:4000/api/v1/version-files/bulk \\
  ...
  -d '{"hashes":["sha256:abc..."],"algorithm":"sha256"}'`}</code>
              </pre>
            </div>
            <div className="grid grid-cols-3 gap-3 text-center">
              {[
                ...
                const Icon = f.icon;
                return (
                  <div key={f.k} className="bg-card rounded-lg border p-3">
                    <Icon className="text-primary mx-auto mb-1 h-4 w-4" />
        ...
        <section className="bg-muted/40 border-t py-10">
          <div className="container">
            <h2 className="mb-6 flex items-center gap-3 text-xl font-black tracking-tight">
              <Search className="text-primary h-5 w-5" /> ENDPOINTS
            </h2>
            <div className="bg-card overflow-hidden rounded-xl border">
              {ENDPOINTS.map((e) => (
                <div
                  key={e.path}
                  className="flex flex-col gap-1 border-b p-4 last:border-b-0 sm:flex-row sm:items-center sm:gap-4"
                >
                  <span
                    className={`inline-flex w-fit shrink-0 items-center rounded-md border px-2 py-0.5 text-xs font-black ${METHOD_COLOR[e.method]}`}
                  >
                    {e.method}
                  </span>
                  <code className="shrink-0 text-sm font-bold">{e.path}</code>
                  <span className="text-muted-foreground text-sm sm:ml-auto sm:text-right">
                    {e.desc}
                  </span>
                </div>
              ))}
            ...
            <p className="text-muted-foreground mt-4 flex items-center gap-2 text-sm">
              ...
              Modpack exports are <code>.mrpack</code>-compatible — importable straight into the
