    ...
    <div className="animate-pulse space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        {[1, 2].map((i) => (
          <Card key={i}>
            <CardContent className="p-6">
              <div className="bg-muted h-[300px] rounded-lg" />
            </CardContent>
          </Card>
        ))}
      </div>
...
export default function DashboardAnalyticsPage() {
  ...
  const topProjects = useMemo(() => {
    const source = (userAnalytics as any)?.topProjects?.length
      ? (userAnalytics as any).topProjects
      : projects;
    return [...source]
        ...
        status:
          p.status === 'PUBLISHED'
            ...
            : p.status === 'DRAFT'
              ...
              : (p.status ?? 'Published'),
        updated:
          p.updated ?? (p.updatedAt ? new Date(p.updatedAt).toLocaleDateString() : 'recently'),
      }));
  ...
  const kpiCards = [
    {
      ...
      value: String(summary.totalProjects ?? stats.totalProjects),
      icon: Download,
      change: `${summary.publishedProjects ?? stats.publishedCount} published`,
      changeType: 'up' as const,
    },
    {
      label: 'Total Downloads',
      value:
        (summary.totalDownloads ?? stats.totalDownloads) >= 1000
          ? `${((summary.totalDownloads ?? stats.totalDownloads) / 1000).toFixed(1)}K`
          : String(summary.totalDownloads ?? stats.totalDownloads),
      icon: Eye,
      change: 'All time',
      changeType: 'up' as const,
    },
    {
      label: 'Published',
      value: String(summary.publishedProjects ?? stats.publishedCount),
      icon: Users,
      change: `${summary.draftProjects ?? stats.draftCount} drafts`,
      changeType: 'up' as const,
    },
    {
      label: 'Views',
      value:
        (summary.totalViews ?? stats.totalViews) >= 1000
          ? `${((summary.totalViews ?? stats.totalViews) / 1000).toFixed(1)}K`
          : String(summary.totalViews ?? stats.totalViews),
      icon: Star,
  ...
  const versionBars = useMemo(() => {
    const byVersion = (topProjectAnalytics as any)?.downloads?.byVersion as
      Array<{ version: string; count: number }> | undefined;
  ...
  const statusPie = useMemo(() => {
    const byStatus = (userAnalytics as any)?.projectsByStatus as
      Array<{ status: string; count: number }> | undefined;
    if (byStatus && byStatus.length > 0) {
      const total = byStatus.reduce((a, b) => a + b.count, 0) || 1;
      const colors: Record<string, string> = {
        PUBLISHED: 'hsl(142.1 70.6% 45.3%)',
        DRAFT: 'hsl(38 92% 50%)',
        ARCHIVED: 'hsl(220 14% 40%)',
      ...
      <div className="mb-8 grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {topProjectId
                ? `Downloads by version for ${topProjects[0]?.name ?? 'top project'} • ${period}`
            ...
            {versionBars ? (
                ...
                {versionBars.map((v) => (
                  <div key={v.label}>
                    <div className="mb-1 flex items-center justify-between text-sm">
                      <span className="truncate font-medium">v{v.label}</span>
                      <span className="text-muted-foreground">
                        {v.count >= 1000 ? `${(v.count / 1000).toFixed(1)}K` : String(v.count)}
                      </span>
                    </div>
              ...
              <div className="space-y-3">
                ...
                <p className="text-muted-foreground pt-2 text-center text-xs">
                  Sample data - publish a version with downloads to see live bars
                </p>
              </div>
            )}
          </CardContent>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-left">
                    <th className="text-muted-foreground w-8 pb-3 text-xs font-medium uppercase tracking-wider">
                      #
                    </th>
                    ...
                    >
                      ...
                      <td className="py-3.5">
                        <div className="flex items-center gap-3">
                          ...
                          >
                            <span className="text-xs font-bold">{project.name[0]}</span>
                          </div>
                      ...
                      <td className="text-muted-foreground hidden py-3.5 text-xs sm:table-cell">
                        {project.updated}
                      </td>
                    </tr>
                  ))}
