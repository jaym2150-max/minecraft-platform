...
export default function AdminPage() {
  ...
  const handleBan = async (userId: string, username?: string) => {
    ...
    try {
      ...
      setUsers((prev: any[]) =>
        prev.map((u: any) => (u.id === userId ? { ...u, banned: true } : u)),
      );
    } catch (err) {
      ...
      setAdminError(
        `Failed to ban ${username ?? userId}: ${(err as Error)?.message ?? 'request failed'}`,
      );
      setUsers((prev: any[]) =>
        prev.map((u: any) => (u.id === userId ? { ...u, banned: false } : u)),
      );
    }
  };
  ...
  const handleUnban = async (userId: string, username?: string) => {
    ...
    try {
      ...
      setAdminError(
        `Failed to unban ${username ?? userId}: ${(err as Error)?.message ?? 'request failed'}`,
      );
      setUsers((prev: any[]) =>
        prev.map((u: any) => (u.id === userId ? { ...u, banned: true } : u)),
      );
  ...
  const handleChangeRole = async (userId: string, role: string, username?: string) => {
    ...
    try {
      ...
      setAdminError(
        `Failed to set ${username ?? userId} role to ${role}: ${(err as Error)?.message ?? 'request failed'}`,
      );
  ...
  const handleChangeTier = async (userId: string, tier: string, username?: string) => {
    ...
    try {
      ...
      setAdminError(
        `Failed to set ${username ?? userId} tier to ${tier}: ${(err as Error)?.message ?? 'request failed'}`,
      );
      setUsers((prev: any[]) =>
        prevTier !== undefined
  ...
  const handleApproveProject = async (projectId: string, title?: string) => {
    ...
    try {
      ...
      setProjects((prev: any[]) =>
        prev.map((p: any) => (p.id === projectId ? { ...p, status: 'PUBLISHED' } : p)),
      );
    } catch (err) {
      setAdminError(
        `Failed to publish ${title ?? projectId}: ${(err as Error)?.message ?? 'request failed'}`,
      );
      setProjects((prev: any[]) =>
        prev.map((p: any) => (p.id === projectId ? { ...p, status: 'PENDING' } : p)),
      );
    }
  };
  ...
  const handleRejectProject = async (projectId: string, title?: string) => {
    ...
    try {
      ...
      setProjects((prev: any[]) =>
        prev.map((p: any) => (p.id === projectId ? { ...p, status: 'REJECTED' } : p)),
      );
    } catch (err) {
      setAdminError(
        `Failed to reject ${title ?? projectId}: ${(err as Error)?.message ?? 'request failed'}`,
      );
      setProjects((prev: any[]) =>
        prev.map((p: any) => (p.id === projectId ? { ...p, status: 'PENDING' } : p)),
      );
  ...
  const handleToggleFeature = async (projectId: string, featured: boolean, title?: string) => {
    if (!confirmDestructive(featured ? 'Unfeature project' : 'Feature project', title ?? projectId))
      return;
    try {
      await adminSdk.updateProjectFeature(projectId, !featured);
      setProjects((prev: any[]) =>
        prev.map((p: any) => (p.id === projectId ? { ...p, featured: !featured } : p)),
      );
    } catch (err) {
      setAdminError(
        `Failed to toggle feature on ${title ?? projectId}: ${(err as Error)?.message ?? 'request failed'}`,
      );
      setProjects((prev: any[]) =>
        prev.map((p: any) => (p.id === projectId ? { ...p, featured } : p)),
      );
    }
  };
  ...
  const handleResolveReport = async (reportId: string, status: 'RESOLVED' | 'DISMISSED') => {
    ...
    try {
      ...
      setAdminError(
        `Failed to ${status.toLowerCase()} report ${reportId}: ${(err as Error)?.message ?? 'request failed'}`,
      );
    }
  };
  ...
  const handleBulkResolve = async (status: 'RESOLVED' | 'DISMISSED') => {
    ...
    if (
      !confirmDestructive(
        `Bulk ${status.toLowerCase()} ${ids.length} reports`,
        `${ids.length} reports`,
      )
    )
      return;
    try {
      setReports((prev: any[]) =>
        prev.map((r: any) => (ids.includes(r.id) ? { ...r, status } : r)),
      );
      ...
      setAdminError(
        `Bulk ${status.toLowerCase()} failed: ${(err as Error)?.message ?? 'request failed'}`,
      );
      fetchReports();
    }
  };
    ...
    ? [
        {
          ...
          value: analytics.projects.total.toLocaleString(),
          change: `+${analytics.newProjectsToday ?? 0} today`,
          icon: Package,
        ...
        },
        {
          ...
          change: `${analytics.users.banned} banned`,
          icon: Users,
          color: 'text-emerald-500 bg-emerald-500/10',
        },
        {
          label: 'Pending Reports',
          value: analytics.reports.pending.toLocaleString(),
          change: `${analytics.reports.total} total`,
          icon: Flag,
        ...
        },
        {
          ...
          change: `${analytics.downloads.total.toLocaleString()} total`,
          icon: Download,
  ...
  return (
    <DashboardLayout>
      <div className="p-6">
        <div className="from-primary/5 to-background -mx-6 -mt-6 mb-6 border-b bg-gradient-to-b px-6 pb-8 pt-6">
        ...
        {adminError && (
          <div
            ...
            className="border-destructive/40 bg-destructive/10 text-destructive mb-4 flex items-start gap-3 rounded-md border px-4 py-3 text-sm"
          >
        ...
        <div className="mb-6 flex items-center gap-2 overflow-x-auto border-b pb-1">
          {TABS.map((tab) => {
            const Icon = tab.icon;
            return (
              <button
                  ...
                  <Card key={stat.label}>
                    <CardContent className="p-6">
                      <div className="flex items-center gap-4">
                        <div
                          className={`h-12 w-12 rounded-xl ${stat.color} flex shrink-0 items-center justify-center`}
                        >
                  ...
                  <div className="space-y-3">
                    {analytics &&
                      [
                        {
                          label: 'Published',
                          count: analytics.projects.published,
                          color: 'bg-emerald-500',
                        },
                        {
                          label: 'Pending',
                          count: analytics.projects.pending,
                          color: 'bg-amber-500',
                        },
                        {
                          label: 'Archived',
                          count: analytics.projects.archived,
                          color: 'bg-slate-500',
                        },
                        {
                          label: 'Rejected',
                          count: analytics.projects.rejected,
                          color: 'bg-red-500',
                        },
                      ].map((s) => (
                  ...
                  {[
                    {
                      ...
                      desc: `${analytics?.projects.pending ?? 0} projects awaiting approval`,
                      tab: 'projects' as AdminTab,
                    },
                    {
                      label: 'Handle Open Reports',
                      desc: `${analytics?.reports.pending ?? 0} reports need review`,
                      tab: 'reports' as AdminTab,
                    },
                    {
                      label: 'View Platform Analytics',
                      desc: 'Traffic and engagement metrics',
                      tab: 'analytics' as AdminTab,
                    },
                    {
                      label: 'Manage Users',
                      desc: `${analytics?.users.total ?? 0} registered users`,
                    ...
                    <button
                      ...
                      className="bg-card hover:bg-muted/50 flex w-full items-center justify-between rounded-lg border p-4 text-left transition-colors"
                    >
                      <div>
                      ...
                      {filteredProjects.map((p: any) => {
                        ...
                        return (
                          <tr key={p.id} className="hover:bg-muted/50 transition-colors">
                            ...
                            <td className="p-4">
                              {p.downloads >= 1000000
                                ? `${(p.downloads / 1000000).toFixed(1)}M`
                                : (p.downloads?.toLocaleString() ?? 0)}
                            </td>
                            <td className="p-4">
                              <span className={`rounded-full px-2 py-1 text-xs ${badge.cls}`}>
                                {badge.label}
                              </span>
                            </td>
                            <td className="p-4">
                              <button
                                onClick={() => handleToggleFeature(p.id, p.featured, p.title)}
                        ...
                        <tr key={u.id} className="hover:bg-muted/50 transition-colors">
                          ...
                          <td className="p-4">
                            <select
                              ...
                              className={`cursor-pointer rounded-full border-0 px-2 py-1 text-xs ${roleBadgeClass(u.role)}`}
                            >
                          ...
                          <td className="p-4">
                            <span
                              className={`rounded-full px-2 py-1 text-xs ${u.banned ? statusBadgeClass('banned') : statusBadgeClass('active')}`}
                            >
                              {u.banned ? 'banned' : 'active'}
                            </span>
                          </td>
                          <td className="text-muted-foreground p-4">{u.joined ?? '—'}</td>
          ...
          <div className="space-y-4">
            {reportStats && (
              <div className="flex gap-4 text-sm">
                ...
                <span>
                  Resolved: <strong>{reportStats.resolved}</strong>
                </span>
                <span>
                  Dismissed: <strong>{reportStats.dismissed}</strong>
                </span>
                <span>
                  Total: <strong>{reportStats.total}</strong>
                </span>
              </div>
            )}
            ...
            {selectedReports.size > 0 && (
              <div className="flex items-center justify-between rounded-lg border border-amber-500/20 bg-amber-500/5 px-4 py-2">
                <span className="text-sm font-medium">{selectedReports.size} selected</span>
                <div className="flex gap-2">
                  <Button
                    size="sm"
                    variant="outline"
                    className="h-7 border-emerald-200 text-emerald-600 hover:bg-emerald-50"
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    className="text-destructive border-destructive/20 hover:bg-destructive/10 h-7"
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-muted/50 border-b">
                        ...
                        <th className="p-4 text-left font-medium">Project / User</th>
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="p-4 text-sm">
                              {r.projectId ? (
                                <Link
                                  href={`/mod/${r.project?.slug ?? r.projectId}`}
              ...
              {[
                {
                  ...
                  change: `${analytics?.downloads.today?.toLocaleString() ?? 0} today`,
                },
                {
                  label: 'Total Users',
                  value: analytics?.users.total?.toLocaleString() ?? '—',
                ...
                },
                {
                  label: 'Pending Reports',
                  value: analytics?.reports.pending?.toLocaleString() ?? '—',
                  change: `${analytics?.reports.total ?? 0} total`,
                },
              ].map((stat) => {
                return (
                  <Card key={stat.label}>
                    ...
                    {[
                      {
                        label: 'Published',
                        count: analytics.projects.published,
                        pct: analytics.projects.total
                          ? Math.round(
                              (analytics.projects.published / analytics.projects.total) * 100,
                            )
                          : 0,
                      },
                      {
                        ...
                        pct: analytics.projects.total
                          ? Math.round(
                              (analytics.projects.pending / analytics.projects.total) * 100,
                            )
                          : 0,
                      },
                      {
                        ...
                        pct: analytics.projects.total
                          ? Math.round(
                              (analytics.projects.archived / analytics.projects.total) * 100,
                            )
                          : 0,
                      },
                      {
                        ...
                        pct: analytics.projects.total
                          ? Math.round(
                              (analytics.projects.rejected / analytics.projects.total) * 100,
                            )
                          : 0,
                      },
                    ].map((s) => (
